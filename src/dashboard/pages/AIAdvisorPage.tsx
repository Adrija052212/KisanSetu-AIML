 ...
 * User questions are sent to the real FastAPI AI backend
 * through sendMessageToAI().
 */
...
export default function AIAdvisorPage() {
  ...
  const ctx = useMemo(() => {
    ...
    return {
      firstName: fullName.split(" ")[0] || "Ramesh",
      location: "Kothur, Ahilyanagar, Maharashtra, India",
      language: "English",
    };
  }, []);

  const [messages, setMessages] = useState<ChatMessageType[]>(() =>
    seedConversation(ctx.firstName)
  );
  ...
  /**
   * Send farmer's question to the real AI backend.
   */
  const send = async (text: string) => {
    ...
    /* Show typing indicator while backend is processing */
    setTyping(true);

    try {
      /* Real AI call */
      const response = await sendMessageToAI(trimmed, ctx);

      /* Add backend's response to the chat */
    ...
    <DashboardLayout>
      <div className="mx-auto flex w-full max-w-[1240px] flex-col gap-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-xl text-[13.5px] text-[#666666]">
              Ask anything about crops, market prices, weather, or farming
          ...
          <div className="flex items-center gap-3.5 rounded-2xl border border-[#BFE3C5] bg-[#F0FAF1] px-5 py-3.5">
            ...
            <div>
              <p className="font-display text-[14px] font-bold text-[#155B32]">
                Powered by AI
              </p>

              <p className="mt-0.5 text-[11.5px] text-[#5B7A63]">
                Simple answers for a better tomorrow.
              </p>
            </div>
          </div>
        </div>
        ...
        <p className="-mt-2 text-center text-[11px] text-[#999999]">
          KisanSetu AI provides general farming guidance. Verify important
          decisions with your local mandi or agriculture officer.
        </p>
      </div>
