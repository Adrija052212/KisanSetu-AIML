...
export interface MandiRow {
  ...
  change: number;
  distance: string;
  /** Approx. daily mandi arrivals in quintals; replace with live mandi feed. */
  arrivals?: number;
  /** Current buyer-demand level for this crop/market. */
  demand?: "Low" | "Medium" | "High";
  /** Estimated transport cost for a standard 1-tonne load. */
...
function buildCrop(key: CropKey): CropData {
  ...
  const arrivalBase = { Ahmednagar: 1240, Pune: 3420, Nashik: 2840, Solapur: 1980, Lasalgaon: 1980 } as Record<string, number>;
  const demandBase = { Ahmednagar: "Medium", Pune: "High", Nashik: "High", Solapur: "Medium", Lasalgaon: "High" } as Record<string, "Low" | "Medium" | "High">;
  const transportBase = { Ahmednagar: 700, Pune: 1900, Nashik: 2200, Solapur: 2600, Lasalgaon: 1650 } as Record<string, number>;
  ...
  const mandis = TOMATO_MANDIS.map((m) => ({
    ...
    arrivals: Math.round((arrivalBase[m.market] ?? 1800) * (key === "onion" ? 1.15 : 1)),
    demand: demandBase[m.market] ?? "Medium",
    transportCost: transportBase[m.market] ?? 1500,
  }));
